...
export class CrosswordLetter {
    ...
    wordNextLetter: CrosswordLetter;
    wordPriorLetter: CrosswordLetter;

    overlappingLetter: CrosswordLetter;

    static fromWord(word: String): CrosswordLetter {
        const wordAsChars = [...word.toLowerCase()].filter((char) => isChar(char));
    ...
    serializeHelper():[number, SerializedWord] {
        let currentLetter:CrosswordLetter = this;
        const details = [];
